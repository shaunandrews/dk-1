---
description: Starting and managing development servers for all repositories
globs: ["**/*"]
---
...
- "Work on Telex" / "block authoring" → telex
...
### Calypso Start Command

The start script uses `yarn start` (not `yarn start:debug`). The `start:debug` command hardcodes a smaller heap size and overrides `NODE_OPTIONS`, so the script sets its own `NODE_OPTIONS=--max-old-space-size=8192` and runs `yarn start` directly.
...
```bash
# Build Jetpack and its dependencies (required before first dev run)
cd repos/jetpack && pnpm jetpack build plugins/jetpack --deps

# Watch for file changes during development (run in a separate terminal)
...
```
🚀 Starting Telex development environment...

This requires Docker for MinIO (S3-compatible storage) and the server containers.
...
# Ensure MinIO is running (required for artefact storage)
pnpm run minio:status   # Check if running
pnpm run minio:start    # Start if needed

# Start all services (interactive CLI)
pnpm dev
```
